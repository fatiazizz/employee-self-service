...
export default function VehicleShow() {
  ...
  const handleStatusChange = async (newStatus: 'approved' | 'declined') => {
    try {
      setProcessing(true);
      await api.post(`/vehicle-request/${data.id}/status`, { status: newStatus });
      setStatus(newStatus);
      setShowModal(false);
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#00000050] bg-opacity-40">
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-lg">
            <h2 className="mb-4 text-lg font-semibold text-gray-800">Change Vehicle Status</h2>
            <p className="mb-4 text-sm text-gray-600">Select the new status for this vehicle request:</p>
