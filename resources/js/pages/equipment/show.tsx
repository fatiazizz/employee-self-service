...
export default function EquipmentShow() {
  ...
  const handleStatusChange = async (newStatus: 'approved' | 'rejected') => {
    try {
      setProcessing(true);
      await api.post(`/equipment-request/${data.id}/status`, { status: newStatus });
      setStatus(newStatus);
      setShowModal(false);
          ...
          <div className="sm:col-span-2">
            ...
            <ul className="mt-2 list-disc list-inside text-gray-900">
              {data.items && data.items.length > 0 ? (
                data.items.map((item: string, idx: number) => <li key={idx}>{item}</li>)
              ) : (
                <li className="text-gray-400">No items listed</li>
              )}
            </ul>
          </div>
        </div>
